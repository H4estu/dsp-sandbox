...
fn write_data<T>(output: &mut [T], channels: usize, complete_tx: &mpsc::SyncSender<()>, signal: &mut signal::Sine<signal::ConstHz>) 
where
...
fn main() {
    ...
    // while sine_signal.next() != 2.0 {
    //     println!("Next frame: {}", sine_signal.next())
    // }
}
